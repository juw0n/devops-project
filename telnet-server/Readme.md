...
1. **Skaffold:** handles the workflow for building, pushing and deploying native applications on local K8s . (https://skaffold.dev/docs/install/)
2. **Container-structure-test:** This command-line tool provides a framework for validating the structure and contents of a container image. It enables users to define and execute tests against a container image to ensure it meets specific requirements and expectations. (https://github.com/GoogleContainerTools/container-structure-test/)


First I install the skaffold and Container-structure-test inside the directory where the yaml files live.
The skaffold.yaml file describes how to build, test, and deploy my application and it usually live in the root of the project directory where the dockerfile live under version control. 
